...
static const item endrecord = { item::endrecord{} };
...
std::ostream& operator<<( std::ostream& stream, const item::endrecord& ) {
    return stream << "/";
}
...
std::ostream& operator<<( std::ostream& stream, const item& x ) {
    struct type : boost::static_visitor< const char* > {
        ...
        const char* operator()( const item::endrecord ) const { return "end"; }
    };
